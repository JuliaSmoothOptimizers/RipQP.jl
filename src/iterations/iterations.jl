...
function compute_αs(x, s_l, s_u, lvar, uvar, Δ, ilow, iupp, n_low, n_upp, n_rows, n_cols)
...
function update_pt_aff!(x_m_l_αΔ_aff, u_m_x_αΔ_aff, s_l_αΔ_aff, s_u_αΔ_aff, Δ_aff, x_m_lvar, uvar_m_x, 
    ...
    s_l_αΔ_aff .= @views s_l[ilow] .+ α_aff_dual .* Δ_aff[n_rows+n_cols+1: n_rows+n_cols+n_low]
    s_u_αΔ_aff .= @views s_u[iupp] .+ α_aff_dual .* Δ_aff[n_rows+n_cols+n_low+1: end]
end

function solve_augmented_system_cc!(Δ_cc, J_fact, Δ_xy, Δ_aff, x_m_lvar, uvar_m_x, rxs_l, rxs_u, 
                                    s_l, s_u, ilow, iupp, n_cols, n_rows, n_low)
...
function update_pt!(x, y, s_l, s_u, α_pri, α_dual, x_m_lvar, uvar_m_x, Δ, lvar, uvar, ilow, iupp, 
    ...
    s_u[iupp] .= @views s_u[iupp] .+ α_dual .* Δ[n_rows+n_cols+n_low+1: end]
end
...
function update_iter_data!(itd, pt, fd, id, safety) 
    ...
    itd.dual_obj = @views fd.b' * pt.y - itd.xTQx_2 + pt.s_l[id.ilow]' * fd.lvar[id.ilow] -
                        pt.s_u[id.iupp]' * fd.uvar[id.iupp] + fd.c0  
    ...
    @inbounds while cnts.k < sc.max_iter && !sc.optimal && !sc.tired # && !small_μ && !small_μ
        ...
        if itd.regu.regul == :classic  # update ρ and δ values, check J_augm diag magnitude 
            out = update_regu_diagJ!(itd.regu, itd.J_augm.nzval, itd.diagind_J, id.n_cols, itd.pdd, 
                                     itd.l_pdd, itd.mean_pdd, cnts, T0) 
            out == one(Int) && break
        end
