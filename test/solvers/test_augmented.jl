@testset "mono_mode" begin
  ...
  if isdefined(HSL, :libhsl_ma57)
    stats2 = ripqp(QuadraticModel(qps2), sp = K2LDLParams(fact_alg = Ma57Fact()), display = false)
    @test isapprox(stats2.objective, -9.99599999e1, atol = 1e-2)
    @test stats1.status == :first_order
  end

  if isdefined(HSL, :libhsl_ma97)
    stats2 = ripqp(QuadraticModel(qps2), sp = K2LDLParams(fact_alg = Ma97Fact()), display = false)
    @test isapprox(stats2.objective, -9.99599999e1, atol = 1e-2)
    @test stats1.status == :first_order
  end
